...
| # | Decision | Choice |
|---|----------|--------|
| **O1** | **Muted event caption under the title** *(extends F2)* *[amended 2026-06-26: line 2 linked]* | A `<p class="splash-meta">` rendered **directly beneath the `JOIN THE MESH` title, above the HUD**, on two lines via a `<br>`: line 1 **`July 8-12, Alte Hölle, Wiesenburg`**, line 2 **`dwebcamp.org`**. Line 1 is **plain muted text**; **line 2 `dwebcamp.org` is a muted link** → `https://dwebcamp.org` (via `config.extra.dwebcamp_url`), `target="_blank" rel="noopener"` (G6/SS-8), styled muted via `.splash-meta a` (overrides the amber link colour so it stays a caption). The link **joins the §2(d) external-link inventory** (a new path on the `dwebcamp.org` host the BUY DEVICE `/tickets` link already uses); it is a **hyperlink, not an asset**, so `check-offline.sh` stays green. Plain hyphen in `8-12` (hacker register). New class **`.splash-meta`** — deliberately **not** `splash-sub` (LP-4 asserts `splash-sub`==0) and carrying no `SYNC`/`START HERE` substring, so **LP-4 still passes**. The blinking title cursor (J4) and the funnel (START / BUY DEVICE / counters — F2/M1/D6) are untouched. |
...
| 5 | Thu Jul 9 10:30-11:30 | Cross-Pollination between P2P & Local First... | X9ENCG | P2P Portal (11) | Networking session | Zenna, Mathias Jud |
...
| 8 | Thu Jul 9 15:00-16:00 | The Dark Horizon Simulation... | V9L89B | Resilience Base (10) | Tabletop simulation | Michael Suantak |
| 9 | Fri Jul 10 10:00-10:30 | Mesh News Network - dWebbing Local Journalism | JFNK39 | Resilience Base (10) | Session *(generic)* | Casey Carr |
...
| **Q2** | **talx-sourced; rooms normalized; missing types not invented** *(extends L2; upholds D12)* | Every rendered value (title, speaker, day, time, room, type, summary) is taken from the linked talx page via a one-time **authoring** fetch — the same "talx is the source of truth" method as L1/L2; **no runtime egress is added** (the built site still never fetches talx; `Details ↗` is a plain hyperlink). **Rooms** normalized to the existing short-name + team-number style: talx's `Decentralized Hardware @ Hackers Lab` → **`Hacker's Lab (7)`** (same venue, existing team number), `Mesh Nest (tent 5)` → **`Mesh Nest (5)`**. New rooms **`P2P Portal`** and **`Resilience Base`** initially rendered with no invented number; **on 2026-07-03 the team supplied their venue numbers → `P2P Portal (11)` and `Resilience Base (10)`** (D12's "later team edit" path — supplied, never invented). **Two sessions have no type on talx**: `V9L89B` → **`Tabletop simulation`** (the word is in its own abstract — sourced-descriptive, like session 1's `Drop-in`); `JFNK39` → the **generic label `Session`** (no talx type, no abstract format word; a neutral placeholder, not a claimed format; flagged in-source for team confirm). Never an invented specific format. |
